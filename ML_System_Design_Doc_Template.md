...
|----|------------|-----------|----------|
| БТ-01 | Система должна определять ориентацию вагона с точностью ≥90% | Критический | ТЗ п.3.1 |
| БТ-02 | Время обработки одного вагона ≤ 30 сек | Критический | ТЗ п.3.2 |
| БТ-03 | Работа в условиях ограниченной видимости (ночь, осадки) | Высокий | ТЗ п.3.3 |
| БТ-04 | Интеграция с АСУ "Транс-Телематика" через API | Критический | ТЗ п.4.1 |
| БТ-05 | Аудит и логирование всех операций | Средний | ТЗ п.4.5 |
...
2. **Организационные:**
   - Срок проведения пилота: 45 календарных дней
   - Бюджет на инфраструктуру: 150 000 руб/мес
   - Доступ к данным: только через защищенный VPN
...
2. Техническая документация (OpenAPI 3.0 спецификация)
...
| ML Model | Нейросетевая модель детекции объектов | mAP@0.5 ≥ 0.95 |
...
- PEP 8, PEP 257 (документирование)
...
| Долг | Причина | Срок погашения | Оценка |
|------|---------|----------------|--------|
| Отсутствие A/B тестирования | MVP фаза | Q3 2026 | 40 ч |
| Hard-coded правила ориентации | Быстрый старт | Q3 2026 | 80 ч |
| Нет распределенного инференса | Ограниченные ресурсы | Q4 2026 | 120 ч |
| Монолитная архитектура | Скорость разработки | Q4 2026 | 160 ч |
...
| Объем выборки | ≥ 10 000 размеченных кадров | Статистическая значимость |
...
1. **Выбор архитектуры:** Сверточные нейросети (CNN) доказали свою эффективность в задачах детекции промышленных объектов (SOTA: YOLOv8, EfficientDet)
2. **Трансферное обучение:** Использование предобученных на ImageNet весов сократит время обучения на 70%
3. **Аугментация данных:** Учет специфики железнодорожного транспорта (снег, дождь, засветы)
